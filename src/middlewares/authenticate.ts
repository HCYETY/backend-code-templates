...
export default async function authenticate (ctx:Context, next:any) {
  // 若用户请求登录或注册，则可以通过
  const dontNeedCheck = ['/api/login', '/api/register', '/api/forget_password', '/api/modify_password', '/api/email'];
  ...
  console.log('invalidJwtTokenArr', invalidJwtTokenArr)
  const isJwt = invalidJwtTokenArr.includes(authorization);
  const jwtInfo = !isJwt && JWT.verify(authorization);
